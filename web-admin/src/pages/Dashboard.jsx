...
import { Users, Home, Banknote, Activity, Heart, Zap, Wrench, Trash2 } from 'lucide-react';
import { PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, ResponsiveContainer, Tooltip as RechartsTooltip, Legend, CartesianGrid } from 'recharts';
...
const Dashboard = () => {
  ...
  if (loading) return <div className="p-8 text-center text-muted">Loading analytics data...</div>;
  ...
  const feeBreakdownData = [
    { name: 'Rent', amount: stats.rentCollected || 0, fill: '#4F46E5' },
    { name: 'Maintenance', amount: stats.maintenanceCollected || 0, fill: '#F59E0B' },
    { name: 'Electricity', amount: stats.electricityCollected || 0, fill: '#3B82F6' },
  ...
  return (
    <div className="fade-in">
      <PageHeader 
        ...
        subtitle="Real-time overview of system statistics, fee collections, and charity fund" 
      />

      {/* KPI Cards */}
      <div className="kpi-grid">
        <Card className="kpi-card">
          <div className="kpi-icon gradient-primary"><Users size={24} color="#FFF"/></div>
        ...
        <Card className="kpi-card">
          <div className="kpi-icon gradient-success"><Banknote size={24} color="#FFF"/></div>
          <div className="kpi-info">
        ...
        <Card className="kpi-card">
          <div className="kpi-icon gradient-warning"><Activity size={24} color="#FFF"/></div>
          <div className="kpi-info">
        ...
        <Card className="kpi-card">
          <div className="kpi-icon" style={{background: 'linear-gradient(135deg, #E11D48, #F43F5E)'}}><Heart size={24} color="#FFF"/></div>
          <div className="kpi-info">
            <p className="kpi-label">Charity Fund</p>
            <h3 className="kpi-value" style={{color: '#E11D48'}}>{formatNPR(stats.charityTotal)}</h3>
          </div>
        </Card>
      </div>
      ...
      <div className="dashboard-grid mt-6">
        ...
        <Card className="col-span-2">
          <h3 className="section-title mb-6">Fee Collection Breakdown (Paid)</h3>
          <div style={{ height: 280 }}>
            <ResponsiveContainer width="100%" height="100%">
      ...
      <Card className="mt-6">
        ...
        <div className="stats-comparison">
          <div className="stat-box border-r">
            ...
            <h2 className="mt-1">{formatNPR((stats.collectedRevenue || 0) + (stats.pendingRevenue || 0))}</h2>
          </div>
          <div className="stat-box border-r">
            <p className="text-muted text-sm uppercase">Tenants per Owner</p>
            <h2 className="mt-1">{stats.ownersCount > 0 ? (stats.tenantsCount / stats.ownersCount).toFixed(1) : 0}</h2>
          </div>
